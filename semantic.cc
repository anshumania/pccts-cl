  ...
  if(counter != numparam ) 
  ...
  else
	{
		...
		for(parameters = a ; parameters !=0 ; parameters = parameters->right, tp=tp->right,parameterNumber++)
		{
			...
			if(tp->kind == "parref" && parameters->ref == 0)
...
//  cout<<"a->tp->kind"<<a->tp->kind<<endl;  // --> error ! create the type
  if(logable(DEBUG))
 	cout<<"creating header for " << a->kind << " and text " << a->down->text << " at line " << a->line << endl;
...
//    cout<<"parameter->text"<<parameters->text<<endl;    
    nextParam = create_type("par"+parameters->kind,0,0);
    // toggle 
  ...
  else if (a->kind=="function")
	{
		...
		*/
		...
		insert_headers(a->down->right->right->down); 
                // type check the procedures and then the instructions just like above
		TypeCheck(child(a,2));
		TypeCheck(child(a,3),"instruction");  // jp31 !!!
...
else if (a->kind == "string")
{
 ...
 // added read,write
  ...
  else if (a->kind == "(")
  {
    ...
    if (logable(DEBUG)) 
   {
	cout<<"*************************[debug] a->down->kind '('"<<a->down->kind<<" a->text "<<a->down->text<<" at line"<<a->line<<endl;
	symboltable.write();
    }
    ...
    {
       if(logable(DEBUG)) 
	cout << " instruction a->line " << a->line << endl;  // jp31 !!
   ...
   else
    {
        if(logable(DEBUG))
		{
		cout << " else a->line " << a->line << endl; //jp31 !!
		cout<<"At line " << a->line <<endl;
		cout<<" a is " << a->text <<endl;
		cout<<" the type of function " << child(a,0)->kind << endl;
	        }

	//not an instruction but a function
	a->tp = symboltable[child(a,0)->text].tp;
	if(logable(DEBUG))
		cout << " not an instruction and not a function " << a->tp->kind << endl;
             
	if(a->tp->kind != "function" )
		{
				errorisnotfunction(a->line); // jp31 !!			
                if(a->tp->kind == "procedure")
			{
				check_params(a->down->right->down,a->tp, a->line,a->tp->numelemsarray);
    ...
    cout<<"BIG PROBLEM! No case defined for kind "<<a->kind << " at line " << a->line<<endl;
  }
